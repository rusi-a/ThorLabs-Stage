...
device_z = setup_device(SERIAL_Z)

threads = [
    threading.Thread(target=home_device, args=(device_x, 'X')),
    threading.Thread(target=home_device, args=(device_y, 'Y')),
    threading.Thread(target=home_device, args=(device_z, 'Z'))
]
for t in threads:
    t.start()
for t in threads:
...
while True:
    try:
        ...
        target_z = float(input(f"Enter Z target (0 to {STAGE_RANGE_MM} mm): "))
        if not (0 <= target_z <= STAGE_RANGE_MM):
            print("Z value out of range.")
            continue

        threads = [
            threading.Thread(target=move_device, args=(device_x, target_x, 'X')),
            threading.Thread(target=move_device, args=(device_y, target_y, 'Y')),
            threading.Thread(target=move_device, args=(device_z, target_z, 'Z'))
        ]
        for t in threads:
            t.start()
        for t in threads:
    ...
    choice = input("Do you want to return all axes to 0 mm (Y/N)? ").strip().upper()
    if choice == 'Y':
        threads = [
            threading.Thread(target=home_device, args=(device_x, 'X')),
            threading.Thread(target=home_device, args=(device_y, 'Y')),
...
print("All stages disconnected. Done.")
